...
convertResultToCsvRecord :: String -> SchnorrSignatureResult -> CsvResult
convertResultToCsvRecord sKey result@(vKey, msg, sig, veriResult) = (sKey, toHex vKey 4,msg, "", toHex sig 4, show veriResult)
...
testVectorsIO = do    
    ...
    (sKey5,vKey5,msg5,mh5,sig5,result5) <- wrongVerificationKeyTestVector1
    (sKey6,vKey6,msg6,mh6,result6) <- verificationKeyNotOnCurveTestVector1
    (sKey7,vKey7,msg7,mh7,sig7,result7) <- wrongMessageRightSignatureTestVector1
    (sKey8,vKey8,msg8,mh8,sig8,result8) <- rightMessageWrongSignatureTestVector1
    ...
    let finalResult = [
            ...
            ("5", sKey5, vKey5, msg5, mh5, sig5, result5, "Wrong Verification key is used to verify signature signed by another signing key. Verification should be false."),
            ("6", sKey6, vKey6, msg6, mh6, "", result6, "Verification key not on the curve. Verification should be false."),
            ("7", sKey7, vKey7, msg7, mh7, sig7, result7, "Wrong message but right signature used. Verification should be false."),
            ("8", sKey8, vKey8, msg8, mh8, sig8, result8, "Right message but wrong signature is used. Verification should be false.")
            ]
    print finalResult
...
signVerifyTestVector1 = do
    ...
    pure $ convertResultToCsvRecord sKey result
...
wrongMessageRightSignatureTestVector1 = do
    ...
    result <- wrongMessageRightSignature sKey msg
    pure $ convertResultToCsvRecord sKeyStr result
...
rightMessageWrongSignatureTestVector1 = do
    let sKeyStr = "9F432B2677937381AEF05BB02A66ECD012773062CF3FA2549E44F58ED240171E"
        msg = "243F6A8885A308D313198A2E03707344A4093822299F31D0082EFA98EC4E6C89"
    sKey <- parseHexSignKey sKeyStr
    result <- rightMessageWrongSignature sKey msg
    pure $ convertResultToCsvRecord sKeyStr result
